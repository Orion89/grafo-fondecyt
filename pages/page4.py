"""
Página 4 — Red de co-ocurrencias de investigadores

Grafo de co-ocurrencias de investigadores en proyectos Fondecyt 2012-2019.
"""
...
dash.register_page(
    __name__,
    path="/page4",
    name="Red de co-ocurrencias de investigadores",
    title="Red de investigadores de proyectos Fondecyt",
    description="Grafo de co-ocurrencias de investigadores en proyectos Fondecyt 2012-2019",
)
...
researchers_network = DashNetwork(
    id="network-2",
    style={"height": graph_data["height"], "width": graph_data["width"]},
    ...
    [
        ...
        dbc.Row(
            [
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H4(
                                            "Co-ocurrencias de investigadores",
                                            className="card-title",
                                        ),
                                        html.H5(
                                            "Redes de colaboración entre académicos",
                                            className="card-subtitle text-secondary",
                                        ),
                                        html.Br(),
                                        html.P(
                                            "Se establece una arista entre dos investigadores si han trabajado en el mismo proyecto. El grosor de la arista refleja el número de colaboraciones.",
                                            className="card-text text-info",
                                        ),
                                        html.P(
                                            "El tamaño de cada nodo refleja el número de sus conexiones: Mientras más conexiones, mayor es el área de cada nodo.",
                                            className="card-text text-info",
                                        ),
                                        html.P(
                                            "Se consideran todos los años del data set en conjunto. No se dibujan los investigadores sin colaboraciones.",
                                            className="card-text text-info",
                                        ),
                                    ]
                                )
                ...
                dbc.Col(
                    [dbc.Spinner(html.Div([researchers_network]), color="info")],
                    width={"size": 10},
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H5(
                            id="title-plot-1", className="text-primary text-center"
                        ),
                        dcc.Graph(
                            id="nodes-comparison-1", config={"displayModeBar": False}
                        ),
                    ],
                    width={"size": 6},
                ...
                dbc.Col(
                    [
                        ...
                        dcc.Graph(
                            id="degree-histogram-1", config={"displayModeBar": False}
                        ),
                    ],
                    width={"size": 6},
...
def node_comparison(selected_node_dict):
    ...
    points_graph = go.Scatter(
        ...
        y=[
            centralities_labels[abr].replace(" ", "<br>")
            for abr in node_statistics.index[:-2]
        ],  # node_statistics.index[:-2],
        name="Investigador",
        marker=dict(
            color="#3394D5",
            line_color="#3394D5",
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, showline=False, tickfont=dict(size=14)),
...
def degree_histogram(selected_node_dict):
    ...
    colors = [
        "#3394D5" if degree == node_degree else "#E6E9EA" for degree in range(0, 30 + 1)
    ]
...
@callback(Output("info-graph-1", "children"), Input("interval-1", "n_intervals"))
